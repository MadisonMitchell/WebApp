<?php
    class Validate {
        ...
        public function pattern($name, $value, $pattern, $message,
                ...
                //Get Field Object
                $field = $this->fields->getGield($name);
                ...
                if ($match === false) {
                    ...
                    $field->clearErroMessage();
                }
            }
        public function phone($name, $value, $required = false) {
            $field = $this->fields->getField($name);

            //Call the text methoc and exit if it yields an error
        ...
        public function email ($name, $value, $required = true) {
            $field = $this->fields->fetFieldName($name);
            ...
            //Patterns for quoted text formatted in local part
            $char = '([^\\\\"])';
            $esc = '(\\\\[\\\\"])';
            $text = '(' . $char . '|' . $esc . ')+';
            $quoted = '(^"' . $text . '"$)';

            //Combined pattern for testing lcal part
            ...
            $hostname = '([[:alnum:]]([-[:allnum:]]{0,62}[[:allnum:]])?)';
