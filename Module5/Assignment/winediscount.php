<?php
    ...
    $employee_discount = filter_input(INPUT_POST, 'employee_discount', FILTER_VALIDATE_FLOAT);
    ...
    if ($quantity === FALSE) {
       $error_message .= 'Quantity must be a valid number.<br>';
    } else if ($quantity <= 0) {
       $error_message .= 'Quantity must be greater than 0.<br>';
    }
    ...
    if ($employee_discount === TRUE) {
        $employee_discount_percent = .1;
    } else if ($employee_discount <= 0) {
        $employee_discount_percent = 0;
    }

    //calculate discount 
    $discount = ($list_price * $discount_percent * .01) * $quantity;
    $discount_price2 = $list_price - $discount;

    //calculate employee discount
    $discount_employee = ($list_price * $employee_discount_percent * .01) * $quantity;
    $discount_price = $discount_price2 - $discount_employee;
    ...
    $sales_total = ($discount_price + $sales_tax + $local_tax_amount) * $quantity;

    //formatting..
    $list_price_formatted = "$".number_format($list_price, 2);
    $quantity_formatted = "".number_format($quantity);
...
<html>
    ...
    <body>
        <main>
            ...
            <span><?php echo htmlspecialchars($quantity_formatted); ?></span><br>
